...
def sync_scores(user=Depends(get_current_user)):
    ...
    def find_history(company_name: str):
        if not company_name:
            return None
    ...
    for idx, p in enumerate(projects.data):
        ...
        breakdown = score_project(
            ...
            key_contacts_count=len(p.get("key_contacts") or []),
    ...
    supabase.table("priority_scores").insert(score_rows).execute()
    supabase.table("weekly_snapshots").upsert(snap_rows, on_conflict="project_id,user_id,week_start").execute()
